...
#define DIE_OPT(msg) \
  ...
  if (ctx.id == -1) {
    if (argc < 1) {
      DIE_USAGE("missing required window title");
    }
    pattern = argv[0];
    if (!pattern || !*pattern) {
      DIE_USAGE("missing required window title");
  ...
  if (argc < 2) {
    DIE_USAGE("missing required window x and y coordinates");
  }
  ...
  argv += 2;
  if (argc == 1) {
    DIE_USAGE("height is required if width is present");
  }
  if (argc > 1) {
    ctx.size.width  = atoi(argv[0]);
    ctx.size.height = atoi(argv[1]);
    if (ctx.size.width <= 0) {
      DIE("width must be positive integer");
    }
    if (ctx.size.height <= 0) {
      DIE("height must be positive integer");
    }
    ctx.hasSize = 1;
  } else {
  ...
  argv += 2;
  if (argc > 0) {
    WARN("ignoring extraneous arguments");
  }
